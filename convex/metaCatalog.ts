...
export const syncCatalogsFromMeta = action({
  ...
  handler: async (ctx, args) => {
    ...
    for (const catalogId of ids) {
      ...
      if (!res.ok) {
        throw new Error(
          `Meta API error: ${res.status} - ${JSON.stringify(data)}`,
        );
      }
...
export const testMetaCatalogToken = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (!token) {
      throw new Error(
        'META_CATALOG_ACCESS_TOKEN no configurado. Usa: npx convex env set META_CATALOG_ACCESS_TOKEN <token>',
      );
    }
...
async function getPropertyPayload(
  ctx: {
    runQuery: (
      query: typeof api.fincas.getById,
      args: { id: Id<'properties'> },
    ) => Promise<PropForCatalog | null>;
  },
  propertyId: Id<'properties'>,
) {
...
export const syncPropertyToCatalogs = action({
  ...
  handler: async (ctx, args): Promise<{ synced: number }> => {
    ...
    const links = (await ctx.runQuery(
      api.propertyWhatsAppCatalog.listByProperty,
      {
    ...
    const prop = (await ctx.runQuery(api.fincas.getById, {
      id: args.propertyId,
    })) as PropForCatalog | null;
